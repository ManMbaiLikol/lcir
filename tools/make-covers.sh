...
#
# Deux vocabulaires, un seul gabarit :
#   - couverture typographique : renseigner « points » et « schéma », laisser
#     « photo » vide. Le schéma se définit dans FIGURES (cover-card.html).
#   - couverture photographique : renseigner « photo », laisser « points » et
#     « schéma » vides. La hauteur suit alors le cadrage de la photo, pour
#     qu'elle ne soit ni recadrée ni déformée.
# Puis relancer. La page de lecture pointe ensuite vers assets/<id>-cover.webp.
...
# identifiant | surtitre | titre | points-clés (séparés par « ; ») | schéma | photo | hauteur
COVERS=(
  "conjoncture-2025|ARTICLE · CONJONCTURE|Conjoncture 2025 : et si le Cameroun comptait son économie selon le SCN 2025 ?|Le PIB n’augmente pas : c’est sa mesure qui change;La valeur du capital naturel n’est jamais additionnée au PIB;Un ratio de dette qui baisse n’est pas un désendettement|conjoncture||1590"
  "minim-martap|ARTICLE · MINES & RESSOURCES|Minim-Martap : la bataille pour Canyon|||assets/minim-martap-photo.jpg|1384"
)
...
for cover in "${COVERS[@]}"; do
  IFS='|' read -r id kicker title points figure photo height <<< "$cover"
  height="${height:-1590}"
  url="http://localhost:$PORT/tools/cover-card.html?kicker=$(urlenc "$kicker")&title=$(urlenc "$title")&points=$(urlenc "$points")&figure=$(urlenc "$figure")&photo=$(urlenc "$photo")&h=$height"
  ...
  "$CHROME" --headless=new --disable-gpu --no-sandbox --hide-scrollbars \
            --force-device-scale-factor=2 --window-size=1240,$((height + 170)) \
            --virtual-time-budget=9000 --screenshot="$TMP/$id.png" "$url" 2>/dev/null
  magick "$TMP/$id.png" -gravity NorthWest -crop 2120x$((height * 2))+0+0 +repage \
